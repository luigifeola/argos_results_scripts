...
def main():
    ...
    if not os.path.isdir(main_folder + '/' + config.folder_experiments):
        ...
        print(colored('ERROR: experiment folder is not an existing directory in result folder: ', 'red') +
              main_folder + '/' + config.folder_experiments)
        exit(-1)
    ...
    if config.baseline_openspace_flag:
        ...
        if not os.path.exists(baseline_dir):
            print(colored('ERROR: baseline folder is not an existing directory in result folder: ', 'red') +
                  baseline_dir)
            exit(-1)
    else:
    ...
    baseline_dir += "/2021-10-13_robots#1_"
    ...
    if not os.path.isdir(results_dir):
        ...
        print(colored('mkdir: ', 'blue') + results_dir)
    else:
    ...
    if config.comparison_plots_flag or config.density_maps_flag:
        ...
        if config.comparison_plots_flag:
            if not os.path.isdir(result_time_dir):
                ...
                print(colored('mkdir: ', 'blue') + result_time_dir)
            else:
                print(colored('WARNING: already exists directory: ' + result_time_dir, 'green'))

            '''***************WMSD Heatmaps***************************************'''
            if config.wmsd_heatmaps_flag:
                if not os.path.isdir(heatmap_dir):
                    os.makedirs(heatmap_dir)
                    print(colored('mkdir: ', 'blue') + heatmap_dir)

                else:
                    print(colored('WARNING: already exists directory: ' + heatmap_dir, 'green'))

                print(colored('MSD Heatmaps Generation ', 'blue'))
                wmsd_heatmaps.evaluate_WMSD_heatmap(main_folder, config.folder_experiments, baseline_dir,
                                                    config.msd_type, heatmap_dir)

        if config.density_maps_flag:
            if not os.path.isdir(density_maps_dir):
                os.makedirs(density_maps_dir)
                print(colored('mkdir: ', 'blue') + density_maps_dir)
            else:
                print(colored('WARNING: already exists directory: ' + density_maps_dir, 'green'))

        print(colored('MSD Evaluation and Density Maps Generation ', 'blue'))
    ...
    if config.cluster_estimation_flag:
        ...
        if not os.path.isdir(cluster_dir):
            ...
            folder_experiment = os.path.join(main_folder, config.folder_experiments)
            print(colored('Cluster estimation study', 'blue'))
    ...
    if config.open_space_flag:
        ...
        if not os.path.isdir(powerlaw_dir):
            ...
            print(colored('Powerlaw: evaluating origin distance distribution', 'blue'))
    ...
    if config.time_stats_flag:
        ...
        if not os.path.isdir(conv_time_dir):
            ...
            print(colored('Convergence time estimation', 'blue'))
        ...
        if not os.path.isdir(ftp_dir):
            ...
            print(colored('First passage time estimation', 'blue'))
            time_stats.evaluate_time_stats(folder_experiment, conv_time_dir, ftp_dir, convergence_time_estimation,
                                           bound_is)
    ...
    if config.generate_pdf_flag:
        print(colored('Generating pdf Plots in folder:', 'blue') + results_dir)
